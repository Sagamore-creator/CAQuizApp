...
extension QuizManager {
    ...
    static func save() {
        let points = quizState.points
        guard let username = AccountManager.loggedInAccount?.username else {
            return
        }
        let gameSave = GameSave(username: username, points: points)
        UserDefaultsManager.saveGame(with: gameSave)
    }

    static func deleteQuestions() {
        UserDefaultsManager.deleteQuestions()
    }
}
